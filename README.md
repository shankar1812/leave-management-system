...
- Attendance regularization request flow
...
| Users | `POST /GET /PUT /api/v1/users` | Admin / HR |
...
| Leave Applications | `POST /PATCH /GET /api/v1/leaves` | Employee / Manager |
...
src/main/resources/
├── application.properties          → Local dev config (values from env vars)
└── application-prod.properties     → Production config for Railway deployment
...
Half-day leaves require `0.5` precision. `int` can't represent this. `double` has floating-point drift that causes incorrect totals over many operations. `BigDecimal` with `scale=1` is the only correct choice for financial/balance calculations.

**Why balance deduction happens before save in `applyLeave()`?**
If save fails after deduction, `@Transactional` rolls back the deduction too. If deduction fails, the application is never saved. Either both succeed or neither does — atomicity guaranteed.

**Why `@TransactionalEventListener(phase = AFTER_COMMIT)` for email?**
Email fires only after the database transaction commits successfully. Without this, a transaction rollback would still send the email — an employee would receive an approval notification for a leave that doesn't exist in the database.

**Why in-memory Caffeine over Redis for caching?**
This is a single-instance deployment. Caffeine is zero-config, sub-millisecond, and sufficient. A Redis-backed cache would be needed for horizontal scaling across multiple instances — documented as a known limitation.
